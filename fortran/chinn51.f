...
c        nmu=96
c
c        version of 03/14/2015.
c        27100 has been corrected.
c        all earlier versions of chinn51.f are wrong and
c        should be discarded, because their 27100 is wrong.
c
c        same as chinn50_alt.f (except for 27100).
c        does all x and w integrations analytic.
c        all mu integrations (also 4000, 4100, 4200, 4300) are done numerically.
c
c        code for the development of the chiral NN pot. at N4LO (=Q^5).
c        started  07/20/2014.
c
...
10011 format (//' chinn51: N4LO code with analytical x and w integr.')
...
c
c
c     !!! We should make a similar section for ce_i here
c     Here it is:
c
c
c        the e_i LECs
      read  (kread, 10003) name,cc
      write (kwrite,10021) name,cc
      ce14=cc(1)*wnq*wn*1.d-9
      ce15=cc(2)*wnq*wn*1.d-9
      ce16=cc(3)*wnq*wn*1.d-9
      ce17=cc(4)*wnq*wn*1.d-9
c
c
...
c     leading minus is because of different convnetion with Kaiser.
      mi=mi+1
      mm=mm+1
      go to 5995
c
...
c                 !!! my guess is that cd145 = d14(lambda) - d15(lambda). Double-check this!
c     leading minus is because of different convnetion with Kaiser.
      mi=mi+1
      mm=mm+1
      go to 5995
...
c        ****************
c
c     1st term [EM, D.16]; '-' in front because signs must be flipped; See 4600 of chiaa() for details
...
c        ****************
c
c     1st term [EM, D.17]; '-' in front because signs must be flipped; See 4700 of chiaa() for details
...
c           **************
c
c           See [Epelbaum, !!! insert a proper reference here] for details
c
c     !!! We don't include 1/m factor, since it'll be included automatically through the 1/M^2 factor in the end, right?
...
c
c           See [Epelbaum, !!! insert a proper reference here] for details 
c           ( -2 factor as in the scanned sheet. !!! double-check this)
...
c
c           See [Epelbaum, !!! insert a proper reference here] for details 
c           ( -2 factor as in the scanned sheet)
...
c           ***************
c     
c     1st term [EM, D.16]; 2nd term [Kaiser, analytical formulas] See 26700 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
26700 c(mm,im) = -(-2.d0/pi) * ( ga4*cb4 / (((4.d0*fpi)**6)*pi*pi) )
c           
c     !!! Leave this as in main chinn50.f
      ic(mi+1,im)=cc(2)
      mi=mi+2
      mm=mm+1
      go to 5995
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 26800 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 26900 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 27000 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c
c************* new attempt at 27100  3/14/2015
c     
27100 c(mm,im) = 1.d0
      ic(mi+1,im)=cc(2)
      mi=mi+2
...
c           ***************
c     
c     1st term [EM, D.16]; 2nd term [Kaiser's analytical formulas]; See 27200 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 27300 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 27400 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 27500 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ***************
c     
c     1st term [EM, D.17]; 2nd term [Kaiser's analytical formulas]; See 27600 of chiaa() for details
c     '-' in front comes from opposite sign conventions in [Kaiser] and [EM]
...
c           ********************
c     
c     1st term [EM, D.17]; 2nd term [eq (1)]; See 27700 of chiaa() for details
...
c           ********************
c     
c     1st term [EM, D.17]; 2nd term [eq (2)]; See 27800 of chiaa() for details
...
c           ********************
c     
c     We don't use c(_,_) for this section; See 27900 of chiaa() for details
27900 c(mm,im) = 1.d0
c     
      mi=mi+1
      mm=mm+1
...
c           ********************
c     
c     We don't use c(_,_) for this section; See 28000 of chiaa() for details
28000 c(mm,im) = 1.d0
c     
      mi=mi+1
      mm=mm+1
...
c
      nmu=96
c
c     !!! What does this cmu do? Do we need it?     
      cmu=1.0d0
c
      if (ez2.eq.0.d0) then
c           [2m_pi, +infinity] case
c           !!! do we really need acut variable anywhere except in mu integration setup?
            acut=1.d0
...
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
            aintmu=0.d0
            do 4015 ii=1,nmu
c                 Now divide by mu^5*(mu^2 + q^2) and integrate. 1/[mu^5*(mu^2 + q^2)] comes from [EM, D.16]
...
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
            aintmu=0.d0
            do 4115 ii=1,nmu
c                 Now divide by mu^5*(mu^2 + q^2) and integrate. 1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17]
c                 1/mu^2 comes from eq 6. PRC 64, 057001
...
c     precalculate ImV(mu)
      if (.not.iv42) then
            iv42 = .true.
            do 4235 ii = 1, nmu
c                 We don't need integration over x here, since it can be easily done analytically in this case.
c                 It gives factor of 2/3.
...
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
            ...
            do 4215 ii=1,nmu
c                 Now divide by mu^5*(mu^2 + q^2) and integrate. 1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17]
c                 1/mu^2 comes from eq 7. PRC 64, 057001
...
c
c                 !!! my guess is that cd12 = d1(lambda) + d2(lambda). Double-check this!     
 4335       continue
      end if
c
c     iterate over output points aa(i), store results in them
...
c
 ...
 4425             continue
c
c     !!! is there a reason why we have aintxia and aintxib separately instead of just one term aintxi?
...
c     iterate over output points aa(i), store results in them
      do 4605 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^5 * q^6, see [EM, D.16]
            aakk=-deltaq(i,1)
...
c     iterate over output points aa(i), store results in them
      do 4705 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^5 * q^4, see [EM, D.17]
            aakk=-deltaq(i,1)
...
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
            aintmu = 0.d0
            do 4715 ii = 1, nmu
c                 Now divide by mu^5*(mu^2 + q^2) and integrate. 
c                 1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17], 1/mu^2 comes from ImVt = ImVs/mu^2
...
c
c           !!! We should be consistent about either using akk*akk or deltaq(i, 4) for q^4 
c                  
            aa(i)=aa(i)*c5*ANS
26105 continue
      mi=mi+1
...
c
c           See [Epelbaum, !!! insert a proper reference here] for details 
c           This section computes Vt4(q). To compute Vs4(q) multiply this section by -q^2 through input data file
c
...
c
c           See [Epelbaum, !!! insert a proper reference here] for details 
c           This section computes Wt4(q). To compute Ws4(q) multiply this section by -q^2 through input data file
c
...
c           ***************
c
c           See [Entem-Machleidt, eq D.16] and [Kaiser, analytical formulas for 6700-7600] for details 
c
26700 continue
c     prepare mu integration. 3-pion case 
...
c     iterate over output points aa(i), store results in them
      do 26705 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^5 * q^6, see [EM, D.16], Kaiser's formulas
            aakk=-deltaq(i,1)
            c5=c(mm,im)*wpi5*(aakk**3)
c
c
...
26705 continue
c
c     !!! Leave this as in main chinn50.f . We don't want to change the input file for version with analytical 6700-7600
      mi=mi+2
      mm=mm+1
      go to 5999
c
...
c           ***************
c
c           This section calculates not Vs13(q) itself, but Vs13(q)/(-q^2) as it comes into Vt13(q).
c           To calculate Vs13(q) itself, multiply this section by -q^2 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
26800 continue
c     prepare mu integration. 3-pion case 
      if (.not.i3ndmu.or.c3c4mu.ne.c4) go to 40
...
c     iterate over output points aa(i), store results in them
      do 26805 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^5 * q^4, see [EM, D.17], Kaiser's formulas
            aakk=-deltaq(i,1)
            c5=c(mm,im)*wpi5*aakk*aakk
c
c           integrate over mu in [3m_pi, +infinity], and store result in aintmu
            ...
            do 26815 ii = 1, n3mu
c                 Now divide by mu^5*(mu^2 + q^2) and integrate. 
c                 1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17], 1/mu^2 comes from Kaiser's formulas
...
c           ***************
c
c           This section only calculates 2nd term of Vt13, [Kaiser's formulas]
c           To calculate whole Vt13(q), add sections 26800 and 26900 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
26900 continue
c     prepare mu integration. 3-pion case 
...
c     iterate over output points aa(i), store results in them
      do 26905 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^3 * q^4, see [EM, D.17], Kaiser's formulas
            aakk=-deltaq(i,1)
...
c           integrate over mu in [3m_pi, +infinity], and store result in aintmu
            aintmu = 0.d0
            do 26915 ii = 1, n3mu
c                 Now divide by mu^3*(mu^2 + q^2) and integrate. 
c                 1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17]
...
c           ***************
c
c           This section calculates not Ws13(q) itself, but Ws13(q)/(-q^2) as it comes into Wt13(q).
c           To calculate Ws13(q) itself, multiply this section by -q^2 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
27000 continue
c     prepare mu integration. 3-pion case 
...
c           ***************
c
c           This section calculates not Vs12(q) itself, but Vs12(q)/(-q^2) as it comes into Vt12(q).
c           To calculate Vs12(q) itself, multiply this section by -q^2 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
27300 continue
c     prepare mu integration. 3-pion case 
...
c           ***************
c
c           This section only calculates 2nd term of Vt12, [Kaiser, 10].
c           To calculate whole Vt12(q), add sections 27300 and 27400 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
27400 continue
c     prepare mu integration. 3-pion case 
...
c           integrate over mu in [3m_pi, +infinity], and store result in aintmu
            aintmu = 0.d0
            do 27415 ii = 1, n3mu
c                 Now divide by mu^3*(mu^2 + q^2) and integrate. 1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17]
...
c           ***************
c
c           This section calculates not Ws12(q) itself, but Ws12(q)/(-q^2) as it comes into Wt12(q).
c           To calculate Ws12(q) itself, multiply this section by -q^2 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
27500 continue
c     prepare mu integration. 3-pion case 
...
c           ***************
c
c           This section only calculates 2nd term of Wt12, [Kaiser, 12].
c           To calculate whole Wt12(q), add sections 27500 and 27600 through input data file.
c           See [Entem-Machleidt, eq D.16, D.17] and [Kaiser, analytical formulas for 6700-7600] for details 
c
27600 continue
c     prepare mu integration. 3-pion case 
...
c           !!! insert proper reference here
c           This section only calculates Vt_class2(q)
c           To calculate Vs_class2(q), multiply this section by -q^2 through input data file.
c
27700 continue
c     prepare mu integration. 2-pion case 
      if (.not.indmu.or.cc4mu.ne.c4) go to 30
...
c     iterate over output points aa(i), store results in them
      do 27705 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^5 * q^4, see [EM, D.17] and eq (1)
            aakk=-deltaq(i,1)
            c5=c(mm,im)*wpi5*aakk*aakk
c
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
            ...
            do 27715 ii = 1, nmu
c                 Now divide by mu^5*(mu^2 + q^2) and integrate.
c                 1/mu^2 comes from eq(1).  1/[mu^3*(mu^2 + q^2)] comes from [EM, D.17]
...
27705 continue
c
c     !!! we use mi=mi+1 here, since we don't use ic(mi+1, im) as in 27600. Is that correct?
      mi=mi+1
      mm=mm+1
      go to 5999
c
...
c
c           !!! insert proper reference here
c           This section only calculates Wt_class2(q)
c           To calculate Ws_class2(q), multiply this section by -q^2 through input data file.
c
27800 continue
c     prepare mu integration. 2-pion case 
      if (.not.indmu.or.cc4mu.ne.c4) go to 30
...
c     iterate over output points aa(i), store results in them
      do 27805 i=1,nt
c           set aakk = q^2 and c5 = overall_const_factor * mpi^5 * q^4, see [EM, D.17] and eq (2)
            aakk=-deltaq(i,1)
            c5=c(mm,im)*wpi5*aakk*aakk
c
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
...
c           f_pi
            fpi = dsqrt(fpi2)
c      
c           overall const factors. 1st term comes from [EM D.16]. Note: we don't use c(mm,im) here.
c           Kaiser sign conventions opposite to ours.
...
c           integrate over mu in [2m_pi, +infinity], and store result in aintmu
            aintmu = 0.d0
            do 27915 ii = 1, nmu
c                 Now divide by mu^5*(mu^2 + q^2), see [EM D.16], and integrate.
                  aintmu = aintmu + wmu(ii)*v79(ii) / 
     1                  (amu4(ii)*amu(ii)*(amu2(ii)+aakk))
27915       continue
c
c           multiply by q^6 * {result of mu integration}, see [EM, D.16] and eq (3)
...
28015       continue
c
c           multiply by q^6 * {result of mu integration}, see [EM, D.16] and eq (4)
            aa(i)=aa(i)*(aakk**3)*aintmu
28005 continue
c
c     !!! we use mi=mi+1 here, since we don't use ic(mi+1, im) as in 27600. Is that correct?
